...
(function () {
    ...
    function askSolver(query) {
        console.log("Start of query #" + (qid++).toString());
    ...
    function urlSyncResolver(url_prefix, progress_callback) {
        return function(fname) {
            ...
            xhr.open("GET", url_prefix + fname, false);
            xhr.send(null);
    ...
    var lazyFS = Driver.lazyFS = { index: fetchJsonSync("fs/index.json"),
                                   files: {},
                                   fs_root: "/fstar/",
                                   url_prefix: "fs/" };
    ...
    Driver.initSMT = function(callback) {
        FStar.SMTDriver.CLI.initAsync(Z3_OPTIONS, callback);
    };
