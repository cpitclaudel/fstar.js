"use strict";
/* global Worker console FStar Mustache */

FStar.IDE = FStar.IDE || {};

(function () {
    var _ = FStar._;
    var debug = FStar.ClientUtils.debug;
    ...
    var Client = FStar.IDE.Client = function (fname, progress_callback) {
        this.fname = fname;
        this.gensym_state = 0;
        ...
        this.onProgress = progress_callback || function(_msg) {};
    ...
    function groupHypothesesByType(hyps) {
        var groups = [];
        var prev_type = null;
        for (var idx = 0; idx < hyps.length; idx++) {
            var hyp = hyps[idx];
            if (hyp.type == prev_type) {
    ...
    Client.prototype.annotateProofState = function(ps) {
        var _this = this;
        var viewHyps = function(hyps) {
            return { "hyps": hyps,
                     "type": hyps[0].type }; };
        var viewGoals = function(goals, label) {
            return _.map(goals, function(g, idx) {
                return { "label": label + (idx + 1),
                         "hyps": _.map(groupHypothesesByType(g.hyps), viewHyps),
                         "goal": g.goal };
            }); };
        var viewProofState = function(ps) {
            return {
                "label": ps.label,
                "location": ps.location && _this.annotateRange(ps.location),
                "goals": viewGoals(ps.goals, "G").concat(viewGoals(ps.smt_goals, "SMT"))
            }; };
        return viewProofState(ps);
    };

    Client.prototype.routeMessage = function(qid, status, data) {
        FStar.ClientUtils.assert(this.callbacks.containsKey(qid), "Missing query ID");
        return this.callbacks.get(qid).call(this, status, data);
    };

    Client.prototype.onMessage = function(event) {
        var kind = event.data.kind;
        var payload = event.data.payload;
        debug("Client.onMessage", kind, payload);
        switch (kind) {
        ...
        case Protocol.Worker.MESSAGE:
            if (payload["query-id"]) {
                ...
                this.routeMessage(payload["query-id"], Protocol.Status.MESSAGE, payload);
            }
            break;
        case Protocol.Worker.EXIT:
            ...
            break;
        default:
            FStar.ClientUtils.assert(false, "Unexpected message kind");
        }
        this.processPending();
    };
    ...
    Client.prototype.annotateRange = function(range) {
    ...
    Client.prototype.makePushCallback = function(snippet) {
        return function(status, payload) {
            ...
            case Protocol.Status.MESSAGE:
                ...
                break;
            default:
                FStar.ClientUtils.assert(false, "Unexpected status");
            }
        };
    };
